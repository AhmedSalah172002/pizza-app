...
import { CREATE_NEW_USER,RESET_PASSWORD,VERIFY_PASSWORD, FOREGT_PASSWORD, GET_CURERNT_USER, LOGIN_USER,EDIT_CURERNT_USER,CHANGE_PASSWORD } from '../type'
...
//edit current  user 
export const editLoggedUser = (data) => async (dispatch) => {
    try {
        const response = await useInsUpdateData(`/api/v1/users/updateMe`,data);
        dispatch({
            type: EDIT_CURERNT_USER,
            payload: response,
            loading: true
        })
...
//change my password
export const changeMyPassword = (data) => async (dispatch) => {
    try {
