...
const ProductCard = ({image ,productId,title ,mediumPrice,description,ratingAvg , favProd}) => {
  ...
  return (
    ...
    <Link onClick={()=>window.scrollTo({ top: 0, behavior: 'smooth' })} to={`/products/${productId}`}>
    ...
    <div className="order-now-btn mt-4 mb-3">
      <Link onClick={()=>window.scrollTo({ top: 0, behavior: 'smooth' })} to="/products/:ProductId">اطلب الأن</Link>
    </div>
  </div>
  )
}
